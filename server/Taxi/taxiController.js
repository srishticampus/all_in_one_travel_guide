...
const viewTaxiBookingByTaxiId = (req, res) => {
  ...
  taxibooking
    .find({ status: "accepted", taxiid: req.params.id, date: { $gte: date } })
    .populate("custid")
    .exec()
    .then((data) => {
      res.json({
...
const viewTaxiBookingReqsByTaxiId = (req, res) => {
  ...
  date.setDate(date.getDate() - 1);
  taxibooking
    .find({ status: "pending", taxiid: req.params.id, date: { $gte: date } })
    .populate("custid")
    .exec()
    .then((data) => {
      res.json({
...
const viewAllTaxiBookingBycustId = (req, res) => {
  ...
  let datas = [];
  taxibooking
    .find({ custid: req.params.id, date: { $gte: dates } })
    .populate("taxiid")
    .exec()
    .then((data) => {
      datas = data;
...
const cancelTaxiBookingByid = async (req, res) => {
  ...
  await taxibooking
    ...
    .then((datas) => {
      console.log(datas);
      if (datas.date.getMonth() > date.getMonth()) flag = 1;
      else if (
        datas.date.getMonth() == date.getMonth() &&
        datas.date.getDate() > date.getDate()
      )
        flag = 1;
...
module.exports = {
  ...
  acceptTaxiBooking,
  bookTaxi,
  viewTaxiBookingByTaxiId,
  viewBookingByid,
  viewAllTaxiBookingBycustId,
  rejectTaxiBooking,
  viewTaxiBookingReqsByTaxiId,
  cancelTaxiBookingByid,
};
