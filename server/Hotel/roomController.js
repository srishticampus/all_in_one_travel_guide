...
const viewRoomNums = (req, res) => {
  room
    .find({ isactive: true }, { _id: 0, roomNo: 1 })
    .exec()
    .then((data) => {
      if (data.length > 0) {
        res.json({
...
const availableRooms = async (req, res) => {
  ...
  await room
    .find({ hotel_id: req.body.hotel_id, type: req.body.type })
    .exec()
    .then((data1) => {
      //console.log(data1);
      data1.map((x) => {
    ...
    .then((data2) => {
      ...
      data2.forEach((elem) => {
        ...
        if (elem.checkindate.getTime() == checkin.getTime()) {
          ...
          (checkout >= elem.checkindate || checkout >= elem.checkoutdate)
        ) {
          console.log(" same 2");
          invalid.push(elem.roomid);
        } else if (
          checkin >= elem.checkindate &&
          (checkout <= elem.checkoutdate || checkin <= elem.checkoutdate)
        ) {
          console.log(" same 3");
          invalid.push(elem.roomid);
...
const viewBookingByCustId = (req, res) => {
  let date = new Date();
  booking
    .find({ custid: req.params.id, checkoutdate: { $gte: date } })
    .populate("hotel_id")
    .populate("roomid")
    .exec()
    .then((data) => {
...
const viewBookingByHotelid = (req, res) => {
  ...
  console.log(date);
  booking
    .find({ hotel_id: req.params.id, checkoutdate: { $gte: date } })
    .populate("custid")
    .populate("roomid")
    .exec()
    .then((data) => {
...
const cancelBookingByid = async (req, res) => {
  ...
  await booking
    ...
    .then((datas) => {
      console.log(datas);
      if (datas.checkindate.getMonth() > date.getMonth()) flag = 1;
      else if (
        datas.checkindate.getMonth() == date.getMonth() &&
        datas.checkindate.getDate() > date.getDate()
      )
        flag = 1;
      console.log(
        "mont",
        datas.checkindate.getMonth(),
        "",
        date.getMonth(),
        "",
        datas.checkindate
